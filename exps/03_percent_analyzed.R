...
plot_generic <- function(in_df, in_xfield, in_xlab, in_commonlab, show_legend=FALSE, show_ylabel=FALSE, in_ylim=20, legend_pos="left") {
  ...
  p = ggplot(in_df, aes_string(in_xfield, "percent_analyzed", fill="variant_zthread")) +
    geom_bar(stat="identity", colour="black", position="dodge") +
    ylim(0,in_ylim) + ylab("Packets analyzed (%)") +
    xlab(in_xlab) +
...
plot_snortconfig <- function(in_df, in_snortconfig="", prefix="", show_legend=FALSE, show_ylabel=FALSE) {
  ...
  return(plot_generic(df, "factor(pkt_size)", xlabel, "Packet size", show_legend, show_ylabel, in_ylim=50))
}
...
plot_snortalert <- function(in_df, in_snortalert="", prefix="", show_legend=FALSE, show_ylabel=FALSE) {
  ...
  return(plot_generic(df, "factor(pkt_size)", xlabel, "Packet size", show_legend, show_ylabel, in_ylim=12))
}
...
plot_pcap <- function(in_df, in_pktgenpcap="", prefix="", show_legend=FALSE, show_ylabel=FALSE) {
  ...
  return(plot_generic(df, "factor(num_rules)", xlabel, "Number of rules", show_legend, show_ylabel, legend_pos="right"))
}
...
dferr = aggregate(percent_analyzed ~ variant + sleep + zthread + pktgenconfig + pktgenpcap + snortconfig + snortrule + snortalert + pkt_size + num_flows + num_rules + variant_zthread, data=df, FUN = function(x) c(mean = mean(x), se = stderror(x)))
dferr <- do.call(data.frame, dferr)
dferr$percent_analyzed.sepercent = dferr$percent_analyzed.se / dferr$percent_analyzed.mean * 100
...
pdf("fig/21_analyzed_pktsize.pdf", width=15, height=3) 
...
pdf("fig/22_analyzed_numflows.pdf", width=15, height=3) 
...
pdf("fig/23_analyzed_numrules.pdf", width=15, height=3) 
...
pdf("fig/24_analyzed_config.pdf", width=8, height=3) 
...
pdf("fig/25_analyzed_alert.pdf", width=8, height=3) 
...
pdf("fig/26_tput_pcap.pdf", width=15, height=3) 
