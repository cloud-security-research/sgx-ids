...
plot_generic <- function(in_df, in_xfield, in_xlab, in_commonlab, show_legend=FALSE, show_ylabel=FALSE, in_ylim=2000, legend_pos="left") {
  ...
  p = ggplot(in_df, aes_string(in_xfield, "mbps_analyzed", fill="variant_zthread")) +
    geom_bar(stat="identity", colour="black", position="dodge") +
    ylim(0,in_ylim) + ylab("Throughput (Mbps)") +
    xlab(in_xlab) +
...
plot_snortconfig <- function(in_df, in_snortconfig="", prefix="", show_legend=FALSE, show_ylabel=FALSE) {
  ...
  return(plot_generic(df, "factor(pkt_size)", xlabel, "Packet size", show_legend, show_ylabel, in_ylim=6800))
}
...
plot_snortalert <- function(in_df, in_snortalert="", prefix="", show_legend=FALSE, show_ylabel=FALSE) {
  ...
  return(plot_generic(df, "factor(pkt_size)", xlabel, "Packet size", show_legend, show_ylabel, in_ylim=1200))
}
...
plot_pcap <- function(in_df, in_pktgenpcap="", prefix="", show_legend=FALSE, show_ylabel=FALSE) {
  ...
  return(plot_generic(df, "factor(num_rules)", xlabel, "Number of rules", show_legend, show_ylabel, in_ylim=1500, legend_pos="right"))
}
...
dferr = aggregate(mbps_analyzed ~ variant + sleep + zthread + pktgenconfig + pktgenpcap + snortconfig + snortrule + snortalert + pkt_size + num_flows + num_rules + variant_zthread, data=df, FUN = function(x) c(mean = mean(x), se = stderror(x)))
dferr <- do.call(data.frame, dferr)
dferr$mbps_analyzed.sepercent = dferr$mbps_analyzed.se / dferr$mbps_analyzed.mean * 100
...
pdf("fig/01_tput_pktsize.pdf", width=15, height=3) 
...
pdf("fig/02_tput_numflows.pdf", width=15, height=3) 
...
pdf("fig/03_tput_numrules.pdf", width=15, height=3) 
...
pdf("fig/04_tput_config.pdf", width=8, height=3) 
...
pdf("fig/05_tput_alert.pdf", width=8, height=3) 
...
pdf("fig/06_tput_pcap.pdf", width=15, height=3) 
